    ...
    for(auto const &nc : D3PP::network::Server::roClients) {
        ...
        if (nc->GetId() != clientId) {
            ...
            if (extVersion == 2) {
                ...
                Packets::SendExtAddPlayerName(concrete, nc->GetPlayerInstance()->GetNameId(), nc->GetLoginName(), Entity::GetDisplayname(nc->GetPlayerInstance()->GetEntity()->Id), dudesMap->Name(), 0);
            }
        } else {
            if (extVersion == 2) {
                Packets::SendExtAddPlayerName(concrete, tempNameId, loginName, prettyName, clientMap->Name(), 0);
