...
# Stupid requirement because the lua5.4 package doesn't include the proper symlink
RUN ln -s /lib/x86_64-linux-gnu/liblua5.4.so.0.0.0 /lib/x86_64-linux-gnu/liblua-5.4.so

WORKDIR /opt/D3PP

COPY build/D3PP D3PP
...
COPY bin/Debug/Plugins/ Plugins/
COPY bin/Debug/Maps/ Maps/
COPY bin/Debug/www www/
